// === Remplace Sidebar.tsx en entier (ajout entrée 02.5 Ebooks) ===
import { NavLink } from "react-router-dom";
import {
  LayoutDashboard, FolderTree, Package, BookOpen, ShoppingCart, Inbox, Users, Settings, Zap, Tag, BarChart2,
} from "lucide-react";
...
export const NAV = [
  ...
  { to: "/ebooks",     label: "Ebooks",      icon: BookOpen,        code: "02.5" }, // NEW
  ...
  { to: "/statistics", label: "Statistiques",icon: BarChart2,       code: "08" },
];
  ...
  return (
    <aside className="hidden lg:flex fixed inset-y-0 left-0 z-30 w-64 flex-col border-r-2 border-sidebar-border bg-sidebar">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        <p className="eyebrow px-3 mb-3" style={{ color: "hsl(var(--muted-foreground))" }}>// Navigation</p>
        {NAV.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
